#!/usr/bin/env bash
# TensorHost — connect your mailbox to the Gemini CLI.
...
command -v gemini >/dev/null 2>&1 || { echo "Gemini CLI not found. Install it: https://github.com/google-gemini/gemini-cli"; exit 1; }
...
echo "== Connecting your TensorHost mailbox to the Gemini CLI =="
mkdir -p "$DIR"
...
# Merge a secret-free mcpServers entry into ~/.gemini/settings.json.
SERVER_NAME="$SERVER_NAME" LAUNCHER_PY="$DIR/launcher.py" VENV_PY="$DIR/.venv/bin/python" \
"$DIR/.venv/bin/python" - <<'TENSORHOST_GEMINI_EOF'
...
if p.exists():
    try:
        ...
        cfg = {}
cfg.setdefault("mcpServers", {})[os.environ["SERVER_NAME"]] = {
    "command": os.environ["VENV_PY"],
    "args": [os.environ["LAUNCHER_PY"]],
}
p.write_text(json.dumps(cfg, indent=2) + "\n")
p.chmod(0o600)
print("Updated " + str(p))
TENSORHOST_GEMINI_EOF

echo
echo "Done. Restart the Gemini CLI (or run /mcp reload), then try: \"list my recent unread\"."
echo "Disconnect later: remove the \"$SERVER_NAME\" entry from ~/.gemini/settings.json (and revoke the app password in webmail)."
printf '%s\n' 'sha256-44f47c50a58f6261ee1d5191' > "$DIR/RELEASE.tmp"
mv "$DIR/RELEASE.tmp" "$DIR/RELEASE"
echo "Installed Mail-MCP release sha256-44f47c50a58f6261ee1d5191. Rerun this installer to update; installations do not auto-update."
